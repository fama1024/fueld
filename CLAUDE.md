...
| Nachfragen (Dashboard) | Freitext-Frage-Karte unter den Nährstoff-Ringen (`AskCard`) mit **gespeichertem Chatverlauf** (Tabelle `assistant_message`, V22). `scope` wird per Toggle **innerhalb der Karte** gewählt ("Tag" / "7 Tage") — unabhängig vom Heute/Woche-Tab der Nährstoff-Ringe; `date` ist der Tag aus der Dashboard Tage-Navigation (Default heute). Thread-Schlüssel = `(user, scope, periodDate)` — `periodDate` ist immer der gewählte Tag; bei `scope=range7` das Ende des rollierenden 7-Tage-Fensters (nicht kalenderwochenbasiert). `POST /api/v1/assistant/ask` schickt Profil + Tagesziele + Log-Einträge des Zeitraums als frischen Kontext zusammen mit den bisherigen Thread-Nachrichten (max. 20, als `system`/`addUserMessage`/`addAssistantMessage` Multi-Turn) an `claude-sonnet-5`; Frage + Antwort werden erst nach erfolgreicher Antwort gespeichert (kein verwaister Eintrag bei Fehlern). `GET /api/v1/assistant/messages?scope=&date=` lädt den Verlauf beim Öffnen der Karte bzw. Toggle-Wechsel. Bisheriger Verlauf liegt standardmäßig eingeklappt unter einem Akkordion ("Bisherige Antworten (n)"), öffnet sich automatisch nach einer neuen Antwort. Profil-/Log-Formatierung weiterhin geteilt mit dem Insight über `com.fueld.ai.LogContextFormatter` |
...
- id (UUID), user_id (FK)
- scope (VARCHAR: `today` | `range7`)
- period_date (DATE) — Thread-Schlüssel: der gefragte Tag (bei `range7` das Ende des 7-Tage-Fensters)
...
3. Mahlzeiten + Trainings des Zeitraums (`date` = gefragter Tag, Default heute; `scope=today` → nur dieser Tag, `scope=range7` → dieser Tag und die 6 davor), Format identisch zum Insight (`LogContextFormatter`)
...
### Assistent
- `POST /api/v1/assistant/ask` — `{ question, scope: "today" | "range7", date? }` → `{ answer, scope, periodDate }`. Frage vom Dashboard, Kontext = Profil + Tagesziele + Log-Einträge des Zeitraums + bisheriger Chatverlauf. `date` = gefragter Tag (Default heute); bei `scope=range7` das Ende des 7-Tage-Fensters. Frage + Antwort werden als `ASSISTANT_MESSAGE` gespeichert
- `GET  /api/v1/assistant/messages?scope=today|range7&date=YYYY-MM-DD` — gespeicherter Chatverlauf für einen Thread (aufsteigend nach Zeit)
